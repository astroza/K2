...
# 1 "/usr/lib/gcc/x86_64-linux-gnu/4.4.4/include/stdarg.h" 1 3 4
# 40 "/usr/lib/gcc/x86_64-linux-gnu/4.4.4/include/stdarg.h" 3 4
...
void task0(kpl_task *t)
{
 void *pos = t->pos; t->pos = 0; if(pos != 0) goto *pos;; {
  kpl_msg *msg;

  t1 = time(((void *)0));

  lb_15: if(!(kpl_msg_recv(&msg))) { t->pos = &&lb_15; printf("WAIT: Task=%d Line=%d\n", kpl_task_get_tid(t), 15); return; };
  printf("Llego mensaje desde %hhu data=%hhu\n", msg->src, msg->data);
  lb_17: if(!((time(((void *)0)) - t1) >= 5)) { t->pos = &&lb_17; printf("WAIT: Task=%d Line=%d\n", kpl_task_get_tid(t), 17); return; };
 }
}

void task1(kpl_task *t)
{
 void *pos = t->pos; t->pos = 0; if(pos != 0) goto *pos;; {
  lb_24: if(!(kpl_msg_send(dst, 88))) { t->pos = &&lb_24; printf("WAIT: Task=%d Line=%d\n", kpl_task_get_tid(t), 24); return; };
  puts("Enviado");
 }
}
